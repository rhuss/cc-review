...
The state file is stored at `.cc-review/.triage-state.json`.
...
Read the configurable threshold from config:
...
3. **Determine author type**: Check the first comment's author. The primary indicator is the GraphQL `... on Bot { id }` fragment: if the author has a `Bot` type in the response, classify as bot. As a secondary fallback (for cases where the GraphQL type fragment is absent), check if the author login ends with `[bot]`. Classify as human only when neither indicator matches. The GraphQL type takes priority.
...
3. If a spec was provided via `--spec`, include relevant spec requirements as context.
...
```
Valid point -- this is out of scope for this PR (<1 sentence why: requires larger refactoring / architectural change / etc>). Tracked for follow-up.

<!-- cc-review-triage:deferred -->
```
...
**Hard rule**: NEVER resolve a thread that has not been replied to with a `<!-- cc-review-triage -->` or `<!-- cc-review-triage:deferred -->` signature. Resolving without a reply silently discards review feedback.
...
When `--spec <path>` is provided, read the spec file at the given path and use it as additional context when assessing bot suggestions in Step 6b.
...
4. **Present for approval**: Present these options to the user:
   - **Approve**: Post the reply as-is (with `<!-- cc-review-triage -->` signature)
...
```bash
AUTO_REMEDIATE=$(resolve_config "triage.codecov.auto_remediate" "true")
PATCH_THRESHOLD=$(resolve_config "triage.codecov.patch_threshold" "80")
```
...
**Mandatory**: This step MUST execute after every triage pass, even when all threads were already handled or all were resolved. This step surfaces patterns and tracks progress. Skipping it silently discards the feedback loop.
...
## Step 14: Capture Deferred and Rejected Findings to Idea Inbox

**This step only runs when `--idea-inbox <path>` is passed.** Without the flag, skip this step entirely.

**Skip this step if**: no comments were deferred AND no comments were rejected, OR `--idea-inbox` was not provided.
...
4. **Write to idea inbox**: For each selected theme, append an entry to the path provided via `--idea-inbox`. Create the directory and file if they don't exist:

   ```bash
   mkdir -p "$(dirname "$IDEA_INBOX_PATH")"
   ```
   ...
   **If the file does not exist**, create it first:
   ...
   Ideas captured from code reviews for future consideration.
   ```
   ...
   **For each selected theme**, append an entry at the end of the file:
   ...
   Report: `Captured N themes to $IDEA_INBOX_PATH`
