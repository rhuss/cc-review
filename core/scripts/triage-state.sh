#!/bin/bash
# triage-state.sh - Manage PR triage state file
#
# Usage:
#   triage-state.sh init <pr_number>
#   triage-state.sh get <pr_number> <comment_id>
#   triage-state.sh set <pr_number> <comment_id> <action> <reply_id>
#   triage-state.sh list-unhandled <pr_number> <comment_ids_json>
#   triage-state.sh cleanup [<pr_number>]
#
# State file: .cc-review/.triage-state.json

set -euo pipefail

STATE_FILE="${TRIAGE_STATE_FILE:-.cc-review/.triage-state.json}"
...
do_init() {
  [ $# -eq 1 ] || { echo "Usage: triage-state.sh init <pr_number>" >&2; exit 2; }
  local pr="$1"
...
do_get() {
  [ $# -eq 2 ] || { echo "Usage: triage-state.sh get <pr_number> <comment_id>" >&2; exit 2; }
...
do_set() {
  [ $# -eq 4 ] || { echo "Usage: triage-state.sh set <pr_number> <comment_id> <action> <reply_id>" >&2; exit 2; }
...
do_list_unhandled() {
  [ $# -eq 2 ] || { echo "Usage: triage-state.sh list-unhandled <pr_number> <comment_ids_json>" >&2; exit 2; }
...
case "${1:-}" in
  ...
  cleanup)
    shift; do_cleanup "$@" ;;
  *)
    echo "Usage: triage-state.sh {init|get|set|list-unhandled|cleanup}" >&2
    exit 2 ;;
esac
